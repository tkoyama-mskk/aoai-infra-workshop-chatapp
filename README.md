...
### STEP5 - 閉域化対応
演習で閉域化を行う場合にはプライベートポイント／仮想ネットワーク統合を有効化を行います。詳しくは演習のテキストをご確認いただくか講師の指示にしたがってください。

#### プライベートポイント
<img src="figs/02-private-endpoint.svg" alt="alt" width="800px" />

- プライベートエンドポイント接続の構成画面で「+追加」を選択して、プライベートエンドポイントを追加します。
なお、構成方法は「簡易」を選択します。
...
#### （補足）マネージドIDの有効化について
本サンプルプログラムはマネージドIDには対応していません。対応する為には以下の設定に加えてプログラムの修正が必要になります。

<img src="figs/02-managedid.svg" alt="alt" width="400px" />
